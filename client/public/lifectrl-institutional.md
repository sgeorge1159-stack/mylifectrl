# LifeCTRL™ Institutional — One-Pager

> The institution pays. The individual receives LifeCTRL as part of their services.

---

## The Problem

Case managers are overloaded. Individuals fall through the cracks between appointments. The "case mgmt stuff that never gets done" — document tracking, deadline reminders, resource coordination, progress monitoring — is exactly where people fail.

LifeCTRL doesn't replace case managers. It operationalizes the work **between** case-management contacts.

---

## How It Works

1. Institution licenses LifeCTRL for their population
2. Individuals receive access as part of their program/services
3. Each person describes their situation in plain English
4. AI generates a personalized, prioritized action plan with tasks, deadlines, resources, and progress tracking
5. Case managers can view dashboards, monitor progress, and step in when needed

---

## Vertical Applications

### LifeCTRL ReEntry (corrections, re-entry, halfway houses)

**Example prompt:** *"I'm being released in 30 days. I need identification, housing, employment, Medicaid, probation reporting, transportation, and substance-use treatment."*

**The plan generates:**
- **Before release:** ID documents, benefits applications, housing waitlists, family contact plan
- **First 24 hours:** Probation check-in, medication pickup, temporary housing confirmation
- **First 7 days:** State ID appointment, Medicaid activation, job readiness intake, SUD treatment intake
- **First 30 days:** Employment applications, permanent housing search, transportation setup, banking
- **90-day stabilization:** Ongoing appointments, benefit renewals, support group schedule

Each task has a deadline, a resource link, and a category. Progress is trackable by both the individual and their case manager.

### LifeCTRL Recovery (SUD/behavioral-health programs)

- Treatment intake timeline
- Medication management schedule
- Counseling/therapy appointment tracking
- Sober living / housing coordination
- Employment / disability benefits navigation
- Relapse prevention plan with trigger tracking

### LifeCTRL Campus (colleges/universities)

- Academic planning and deadline tracking
- Financial aid / FAFSA navigation
- Housing and roommate coordination
- Mental health resource directory
- Internship/career prep timeline

### LifeCTRL Workforce (workforce-development programs)

- Job readiness checklist
- Resume and interview prep timeline
- Training/certification tracking
- Transportation and childcare coordination
- Benefits cliff management (transitioning off assistance)

### LifeCTRL Care (healthcare/care navigation)

- Appointment and medication tracking
- Insurance claim organization
- Specialist referral coordination
- Chronic condition management plans
- Caregiver support and respite scheduling

---

## Pricing & Packages

### Starter — For Small Programs & Nonprofits

| | |
|---|---|
| **Best for** | Nonprofits, small re-entry programs, community organizations |
| **Seats** | Up to 50 active individuals |
| **Price** | **$500/month** (≈$10/seat) |
| **Includes** | Full LifeCTRL platform, 5 program-specific Life Kits, case manager dashboard, basic analytics |
| **Annual** | $5,000/year (save 17%) |

---

### Professional — For Mid-Size Facilities & Programs

| | |
|---|---|
| **Best for** | County jails, treatment centers, workforce programs, university departments |
| **Seats** | Up to 500 active individuals |
| **Price** | **$3,500/month** (≈$7/seat at capacity) |
| **Includes** | Everything in Starter + unlimited Life Kits, custom program templates, priority AI generation, API access, dedicated onboarding |
| **Annual** | $35,000/year (save 17%) |

---

### Enterprise — For State Agencies & Large Institutions

| | |
|---|---|
| **Best for** | State DOCs, BOP, large health systems, statewide workforce agencies |
| **Seats** | Unlimited |
| **Price** | **$8,500–15,000/month** (based on population size) |
| **Includes** | Everything in Professional + white-label option, SSO integration, dedicated success manager, custom AI model fine-tuning for program-specific guidance, SLA, compliance documentation |
| **Annual** | $85,000–150,000/year |

---

### Pilot Program — Risk-Free Trial

| | |
|---|---|
| **Best for** | Organizations evaluating LifeCTRL before full commitment |
| **Duration** | 90 days |
| **Seats** | Up to 100 individuals |
| **Price** | **$2,500** (one-time) |
| **Includes** | Full Professional tier features, onboarding support, end-of-pilot impact report |
| **Conversion** | 50% of pilot fee credited toward annual Professional or Enterprise license |

---

### Government RFPs

For Federal BOP, HHS, VA, state-level master contracts, or grant-funded programs — priced per eligible population via standard procurement process. We participate in SAM.gov and can respond to RFPs with full compliance documentation.

---

> All prices are projected and subject to adjustment based on early customer discovery and pilot data. Volume discounts available for multi-facility deployments.

---

## What a Pilot Looks Like

1. **Discovery call** — understand their population, pain points, current case mgmt gaps
2. **Scoped pilot** — 30–90 days, one facility or program, 50–200 individuals
3. **Success metrics** — plan completion rate, task adherence, case manager time saved, recidivism/readmission impact
4. **Expansion** — from pilot to full license based on results

---

## FAQs for Institutional Buyers

**Q: Does this replace our case managers?**
No. LifeCTRL handles the routine administrative coordination between contacts. Case managers focus on high-value interventions.

**Q: Is the data secure?**
Yes. Each individual's LifeVault is encrypted. Data is never used for training or shared with third parties.

**Q: What if someone doesn't have a smartphone?**
Plans can be printed. The institution's staff can input situations on behalf of individuals. Tablet/kiosk access is an option.

**Q: Can we customize plans for our program?**
Yes. Institutions can create program-specific Life Kits with pre-built templates, required tasks, and local resource directories.

**Q: How is this different from existing case management software?**
Existing tools are databases for staff. LifeCTRL is a personal assistant for the individual — it meets them where they are, in their own words, and gives them a roadmap they can follow themselves.

---

## Contact

For institutional inquiries: `hello@mylifectrl.com`
